# Harness Worksheet: Provider Third Capability
...
- user_visible_change: `rfs provider run qa_claw check_authz_consistency` becomes available as the third read-only provider capability

## 2. Scope

- in:
  - add `check_authz_consistency` to the qa_claw allowlisted runtime surface
  - keep the capability argument-free and read-only
  - preserve the existing setup/status/run UX
- out:
  - no fourth qa_claw capability yet
  - no NestClaw runtime execution
  - no generic provider argument expansion
- assumptions:
  - `security/check-authz-matrix-consistency.py` remains the source of truth for this verification rule
...
- runtime_signals:
  - authz consistency capability runs without extra args
  - failures map to `AUTHZ_MISMATCH`
...
- escalation_conditions:
  - authz consistency requires richer config than repo root alone
  - capability starts to leak raw traces or unbounded output
- rollback_or_recovery_path:
  - remove only the authz consistency capability and keep the prior read-only set
- promotion_gate:
  - third capability must remain read-only, allowlisted, bounded, and test-covered

## 6. Drift and hygiene

- likely_drift_points:
  - backend security script paths may evolve independently
  - error messaging may drift from provider boundary docs
- scheduled_cleanup_rule:
  - pick the fourth capability only after the third capability remains stable across the next pass
- candidate_future_automation:
  - add tighter contract tests for provider error-code mapping
