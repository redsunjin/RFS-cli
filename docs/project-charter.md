...
Start with configuration, auth/cache boundaries, a local token flow, a read-only metadata adapter, and a local cache boundary before expanding into broader remote sync behavior.
